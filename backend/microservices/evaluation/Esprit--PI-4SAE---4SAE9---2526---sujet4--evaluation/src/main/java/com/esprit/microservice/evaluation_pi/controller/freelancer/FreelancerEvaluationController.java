...
public class FreelancerEvaluationController {
    ...
    @GetMapping("/count/{email:.+}")  // ✅ :.+ pour accepter @
    ...
    @GetMapping("/average-rating/{email:.+}")  // ✅ :.+ pour accepter @
    ...
    public ResponseEntity<?> getEvaluationDetails(
        ...
        if (!e.getEvaluatedId().equalsIgnoreCase(freelancerEmail))
    ...
    public ResponseEntity<?> respondToEvaluation(
    ...
    @PostMapping("/evaluate-client/{clientEmail:.+}")  // ✅ :.+ pour accepter @
    ...
    @GetMapping("/freelancer/{freelancerId:.+}/given")  // ✅ :.+ pour accepter @
    ...
    @GetMapping(value = "/stats/{email:.+}/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)  // ✅ :.+ pour accepter @
    public SseEmitter streamStats(@PathVariable String email) {
        SseEmitter emitter = new SseEmitter(1800000L);
        List<SseEmitter> list = emitters.computeIfAbsent(
                email.toLowerCase(), k -> new CopyOnWriteArrayList<>()
        );
        list.add(emitter);
        emitter.onCompletion(() -> list.remove(emitter));
        emitter.onTimeout(() -> list.remove(emitter));
        emitter.onError(ex -> list.remove(emitter)); // ✅ cleanup sur erreur aussi
    ...
    private Map<String, Object> buildDetailedStats(String email) {
        ...
        stats.put("averageRating",
                Math.round(evals.stream()
                        .filter(e -> e.getRatingGlobal() != null)
        ...
        for (int i = 1; i <= 5; i++) {
            final int s = i;
            dist.put(String.valueOf(s), evals.stream()
                    .filter(e -> e.getRatingGlobal() != null && Math.round(e.getRatingGlobal()) == s)
                    .count());
        }
